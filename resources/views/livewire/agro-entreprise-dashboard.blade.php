                    ...
                    <div class="flex flex-col flex-1 gap-3">
                        <a href="/" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-400 hover:bg-opacity-25 rounded-2xl">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="margin-right: 80 px">
                                <path fill="currentColor" fill-rule="evenodd" d="M11.293 3.293a1 1 0 0 1 1.414 0l6 6l2 2a1 1 0 0 1-1.414 1.414L19 12.414V19a2 2 0 0 1-2 2h-3a1 1 0 0 1-1-1v-3h-2v3a1 
                                1 0 0 1-1 1H7a2 2 0 0 1-2-2v-6.586l-.293.293a1 1 0 0 1-1.414-1.414l2-2z" clip-rule="evenodd" />
                            </svg>
                            Home
                        </a>
                        ...
                        <div class="bg-yellow-500 text-white p-4 rounded-lg shadow-lg">
                            Vous devez d'abord <a href="{{ route('companies.create') }}" class="underline">enregistrer votre entreprise</a> pour créer une publicité.
                        </div>
                        @else
                        <a href="{{ route('advertisements.create') }}" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-400 hover:bg-opacity-25 rounded-2xl">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32 32" style="margin-right: 8px">
                            ...
                            <div class="mt-4">
                                <h2 class="text-xl font-semibold mb-2">Mes Publicités</h2>
                                <ul>
                                    @foreach ($advertisements as $advertisement)
                                    <li class="mb-2">
                                        <div class="border p-4 rounded-lg">
                                            <p>{{ $advertisement->content }}</p>
                                            <p>Début: {{ $advertisement->startDate }}</p>
                                            <p>Fin: {{ $advertisement->endDate }}</p>
                                            <p>Coût: {{ $advertisement->cost }} €</p>
                                            <button wire:click="editAdvertisement({{ $advertisement->id }})" class="bg-yellow-500 text-white px-4 py-2 rounded-lg">Modifier</button>
                                            <button wire:click="deleteAdvertisement({{ $advertisement->id }})" class="bg-red-500 text-white px-4 py-2 rounded-lg">Supprimer</button>
                                        </div>
                                    </li>
                                    @endforeach
                                </ul>
