...
<div x-data="{ tab: 'TOUT LES PRODUITS', tabWidth: 0, tabLeft: 0 }" class="relative w-full max-w-lg mx-auto mt-10">
    <div class="flex justify-around text-gray-600">
        <div class="category-tab cursor-pointer" :class="{ 'text-green-600': tab === 'TOUT LES PRODUITS' }" @click="tab = 'TOUT LES PRODUITS'; updateProgress($event); filterProducts('')">TOUT LES PRODUITS</div>
        @foreach($productTypes as $type)
            <div class="category-tab cursor-pointer" :class="{ 'text-green-600': tab === '{{ $type->name }}' }" @click="tab = '{{ $type->name }}'; updateProgress($event); filterProducts('{{ $type->name }}')">{{ $type->name }}</div>
        @endforeach
    </div>
...
<script>
    ...
    function filterProducts(category) {
        const products = document.querySelectorAll('.category-items');
...
<section id="Projects" class="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
    @foreach ($products as $product)
        @if (!Str::startsWith($product->picture, '/image'))
            <div class="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl category-items {{ $product->productType->name }}">
                <img src="{{ Storage::url($product->picture) }}" alt="image" class="h-80 w-auto object-cover rounded-t-xl" />
                <div class="px-4 py-3 w-72">
                    <p class="text-lg font-bold text-black truncate block capitalize">{{$product->name}}</p>
                    <div class="flex items-center">
                        <p class="text-lg font-semibold text-black cursor-auto my-3">{{$product->unit_price}} FCFA</p>
                        <div class="ml-auto">
                ...
                <div class="px-4 py-3 mt-auto">
                    <div class="flex items-center">
                        ...
                        <p class="ml-2 text-sm font-semibold text-black">{{$product->user->name}}</p>
                    </div>
                </div>
            </div>
        @endif
