...
<section id="Projects" class="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
    ...
    <div class="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl category-items VEGETABLES flex flex-col h-30">
        <div class="relative flex-grow">
            <img src="{{ $product->picture }}" alt="image" class="h-50 w-full object-cover rounded-t-xl" />
        </div>
        <div class="px-4 py-3">
            <div class="flex flex-col flex-grow">
                <p class="text-lg font-bold text-black truncate capitalize">{{$product->name}}</p>
                <div class="flex items-center justify-between mt-2">
                    <p class="text-lg font-semibold text-black cursor-auto">{{$product->unitPrice}} FCFA</p>
                    <div class="ml-auto">
